...
function content_checking(type,user,event)
{
    if(type=="zip")
    {
	    ...
	    $("#autocomplete_append").scrollTop(0);
        if (event.keyCode == 13)
        {
            if($("#zip_values_autocomplete #autocomplete_append .autocomplete-suggestion").length ==1)
            {
                ...
                var zipcode_city =search_val.substring(name.length,search_val.length)+ city_chk.toUpperCase();
...
function zip_code_geo(city_chk){
    var zip_value = $("#zip_values").val()+ (city_chk.toUpperCase().replace(/-/g , ", "));
    if (zip_value !="" && zip_value!="Enter city (or) Zip code" && !isNaN(zip_value)){
        ...
        for(var i=0; i<ca.length; i++)
        {
            var c = ca[i].trim();
            if (c.indexOf(name)==0){
                var zipcode_city = c.substring(name.length,c.length)+ (city_chk.toUpperCase().replace(/-/g , ", "));
                var city_name = zipcode_city.split(",")
                ...
                {
                    if (newzpcity!='Chennai'){
                        $("#zip_values").val(newzpcity);
                    }else{
                        if(newzpcity.indexOf(", CA") == -1) {
                            newzpcity =  newzpcity
                        }
                        $("#zip_values").val(newzpcity);
                    }
        ...
        return "Walnut Creek" + (city_chk.toUpperCase().replace(/-/g , ","));
    }
    return false;
}
